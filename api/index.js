...
app.get('/api/auth/me', async (req, res) => {
  ...
  try {
    ...
    console.error('❌ Ошибка в /api/auth/me:', err);
    res.status(500).json({ error: err.message });
  }
});

app.post('/api/auth/updateServers', async (req, res) => {
  console.log('📥 POST /api/auth/updateServers', req.body);
  try {
    const token = req.headers.authorization?.replace('Bearer ', '');
    const user = await getUserFromToken(token);
    ...
    console.error('❌ Ошибка в /api/auth/updateServers:', err);
    res.status(500).json({ error: err.message });
  }
});
...
app.post('/api/auth/avatar', async (req, res) => {
  ...
  try {
    ...
    console.error('❌ Ошибка в /api/auth/avatar:', err);
    res.status(500).json({ error: err.message });
  }
});
...
app.post('/api/orders', upload.single('image'), async (req, res) => {
  ...
  try {
    ...
    await supabase.from('orders').insert({ nickname, contacts, prompt, image_url: imageUrl });
    
    const { data: emailData, error: emailError } = await resend.emails.send({
      from: 'onboarding@resend.dev',
      to: 'ukiteroa@gmail.com',
    ...
    if (emailError) console.error('❌ Ошибка Resend:', emailError);
    else console.log('✅ Письмо отправлено:', emailData);
...
app.get('/api/messages', async (req, res) => {
  ...
  try {
    ...
    console.error('❌ Критическая ошибка в GET /api/messages:', err);
    res.status(500).json({ error: err.message });
  }
});
...
app.post('/api/messages', async (req, res) => {
  ...
  try {
    ...
    console.error('❌ Критическая ошибка в POST /api/messages:', err);
    res.status(500).json({ error: err.message });
  }
});
...
app.get('/api/trades', async (req, res) => {
  console.log('📥 GET /api/trades', req.query);
  try {
    ...
    console.error('❌ Критическая ошибка в GET /api/trades:', err);
    res.status(500).json({ error: err.message });
  }
});

app.post('/api/trades', async (req, res) => {
  console.log('📥 POST /api/trades', req.body);
  try {
    const token = req.headers.authorization?.replace('Bearer ', '');
    const user = await getUserFromToken(token);
    if (!user) return res.status(401).json({ error: 'Unauthorized' });
    ...
    console.error('❌ Критическая ошибка в POST /api/trades:', err);
    res.status(500).json({ error: err.message });
  }
});

app.delete('/api/trades/:id', async (req, res) => {
  console.log('📥 DELETE /api/trades/:id', req.params);
  try {
    const { id } = req.params;
    const token = req.headers.authorization?.replace('Bearer ', '');
    const user = await getUserFromToken(token);
    ...
    console.error('❌ Критическая ошибка в DELETE /api/trades:', err);
    res.status(500).json({ error: err.message });
  }
});
...
app.get('/api/stocks', async (req, res) => {
  console.log('📥 GET /api/stocks', req.query);
  res.json([]);
});

// === Заглушка для Google ===
app.post('/api/auth/google', async (req, res) => {
  console.log('📥 POST /api/auth/google', req.body);
